...
async function analyzeAnomaly(runtime2, context) {
  const httpClient = new ClientCapability3;
  runtime2.log("[AI Guardian] ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓");
  runtime2.log("[AI Guardian] ┃  ANOMALY ANALYSIS PIPELINE  [CRE] ┃");
  runtime2.log("[AI Guardian] ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛");
  runtime2.log(`[AI Guardian] Step A │ Price Signal`);
  runtime2.log(`[AI Guardian]         ├ Previous:  $${context.previousPrice.toFixed(4)} (${(context.previousPrice * 100).toFixed(1)}%)`);
  runtime2.log(`[AI Guardian]         ├ Current:   $${context.currentPrice.toFixed(4)} (${(context.currentPrice * 100).toFixed(1)}%)`);
  runtime2.log(`[AI Guardian]         └ Swing:     ${(context.priceDrop * 100).toFixed(2)}% ⚠️ ANOMALY THRESHOLD CROSSED`);
  runtime2.log(`[AI Guardian] Step B │ Community Signals from Polymarket Gamma API`);
  if (context.recentComments.length === 0) {
    runtime2.log(`[AI Guardian]         └ No comments available`);
  } else {
    runtime2.log(`[AI Guardian]         ├ ${context.recentComments.length} comments fetched`);
    context.recentComments.slice(0, 3).forEach((c, i2) => {
      const preview = c.body.length > 80 ? c.body.slice(0, 80) + "..." : c.body;
      const isLast = i2 === Math.min(2, context.recentComments.length - 1);
      runtime2.log(`[AI Guardian]         ${isLast ? "└" : "├"} [${c.createdAt.slice(0, 10)}] ${c.author}: "${preview}"`);
    });
    if (context.recentComments.length > 3) {
      runtime2.log(`[AI Guardian]           (+ ${context.recentComments.length - 3} more)`);
    }
  }
  const prompt = buildPrompt(context);
  runtime2.log(`[AI Guardian] Step C │ Prompt Built`);
  runtime2.log(`[AI Guardian]         ├ Model:    ${context.geminiModel}`);
  runtime2.log(`[AI Guardian]         ├ Tool:     googleSearch (grounding enabled)`);
  runtime2.log(`[AI Guardian]         └ Signals:  ${(context.priceDrop * 100).toFixed(2)}% swing + ${context.recentComments.length} community comments`);
  runtime2.log(`[AI Guardian] Step D │ Calling Gemini API via CRE HTTP Capability...`);
  try {
    const apiKeySecret = runtime2.getSecret({ id: "GEMINI_API_KEY" }).result();
    const geminiApiKey = apiKeySecret.value;
    runtime2.log(`[AI Guardian]         ├ Secret retrieved: ${geminiApiKey ? `✓ (${geminiApiKey.length} chars)` : "✗ EMPTY"}`);
    ...
    const responseBody = httpClient.sendRequest(runtime2, (sendRequester) => {
    ...
    const wrapper = JSON.parse(responseBody);
    runtime2.log(`[AI Guardian]         ├ HTTP Status: ${wrapper._statusCode}`);
    runtime2.log(`[AI Guardian]         ├ Response body (first 300 chars): ${wrapper._body.slice(0, 300)}`);
    ...
    const parsed = JSON.parse(wrapper._body);
    const candidate = parsed?.candidates?.[0];
    const rawText = candidate?.content?.parts?.map((p) => p.text ?? "").join("") ?? "";
    runtime2.log(`[AI Guardian]         ├ Raw text (first 200 chars): ${rawText.slice(0, 200)}`);
    let decision = {};
    try {
    ...
    const sources = (meta?.groundingChunks ?? []).map((c) => c.web?.title ? `${c.web.title} (${c.web.uri})` : c.web?.uri ?? "").filter(Boolean).slice(0, 5);
    runtime2.log(`[AI Guardian] Step E │ Gemini Response Received`);
    if (searchQueries.length > 0) {
      runtime2.log(`[AI Guardian]         ├ Google Search Queries issued by Gemini:`);
      searchQueries.forEach((q, i2) => {
        const isLast = i2 === searchQueries.length - 1 && sources.length === 0;
        runtime2.log(`[AI Guardian]         ${isLast ? "└" : "├"}   \uD83D\uDD0D "${q}"`);
      });
    } else {
      runtime2.log(`[AI Guardian]         ├ No search queries issued`);
    }
    if (sources.length > 0) {
      runtime2.log(`[AI Guardian]         ├ Grounding Sources Used:`);
      sources.forEach((s, i2) => {
        runtime2.log(`[AI Guardian]         ${i2 === sources.length - 1 ? "└" : "├"}   \uD83D\uDCF0 ${s}`);
      });
    }
    ...
    runtime2.log(`[AI Guardian] Step F │ Verdict  [ran by: CRE]`);
    runtime2.log(`[AI Guardian]         ├ ${emoji} Recommendation: ${rec}`);
    runtime2.log(`[AI Guardian]         ├ Confidence:     ${(conf * 100).toFixed(0)}%`);
    runtime2.log(`[AI Guardian]         └ Reason:         ${reason}`);
    runtime2.log("[AI Guardian] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ (end)");
    return { recommendation: rec, confidence: conf, reason, groundingSources: sources, searchQueries, ranBy: "CRE" };
  } catch (e) {
    const errMsg = e?.message ?? String(e);
    runtime2.log(`[AI Guardian] Step E │ ⚠ Gemini API call failed`);
    runtime2.log(`[AI Guardian]         └ Error: ${errMsg.slice(0, 150)}`);
    runtime2.log(`[AI Guardian] Step F │ Verdict (fallback)  [ran by: CRE]`);
    runtime2.log(`[AI Guardian]         ├ \uD83D\uDFE1 Recommendation: MONITOR`);
    runtime2.log(`[AI Guardian]         ├ Confidence:     50%`);
    runtime2.log(`[AI Guardian]         └ Reason:         AI unavailable — manual review recommended`);
    runtime2.log("[AI Guardian] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ (end)");
    return { recommendation: "MONITOR", confidence: 0.5, reason: `AI unavailable: ${errMsg.slice(0, 80)}`, ranBy: "CRE" };
  }
}
function buildPrompt(ctx) {
  const commentLines = ctx.recentComments.slice(0, 10).map((c) => `[${c.createdAt.slice(0, 10)}] ${c.author}: ${c.body}`).join(`
`);
...
async function onCronTrigger(runtime2) {
  ...
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  runtime2.log("[Sonder] CRE cron triggered — fetching Polymarket price...");
  const POLYGON = ClientCapability.SUPPORTED_CHAIN_SELECTORS["polygon-mainnet"];
  const evmClient = new ClientCapability(POLYGON);
  const httpClient = new ClientCapability3;
  const priceUrl = `https://clob.polymarket.com/midpoint?token_id=${config.polymarketTokenId}`;
  const responseBody = runtime2.runInNodeMode((nodeRuntime) => {
  ...
  try {
    ...
    runtime2.log(`[ERROR] Bad Polymarket response: ${responseBody}`);
    return "ERROR: bad polymarket response";
  }
  const price6dec = Math.round(midFloat * 1e6);
  runtime2.log(`[Step 1] Polymarket mid: $${midFloat} → ${price6dec} (6 dec)`);
  ...
  const readResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      ...
      to: config.priceOracleAddress,
      data: getCalldata
    })
  }).result();
  let prevPrice6dec = 0;
  if (readResult.data && readResult.data.length > 0) {
    const [decodedPrice] = decodeAbiParameters(parseAbiParameters("uint256"), toHex(readResult.data));
    prevPrice6dec = Number(decodedPrice);
  }
  const priceDrop = prevPrice6dec > 0 ? (prevPrice6dec - price6dec) / prevPrice6dec : 0;
  const priceSwing = Math.abs(priceDrop);
  const swingDir = priceDrop >= 0 ? "↓" : "↑";
  runtime2.log(`[Step 2] On-chain: ${prevPrice6dec} | New: ${price6dec} | ${swingDir} Swing: ${(priceSwing * 100).toFixed(2)}%`);
  ...
  evmClient.writeReport(runtime2, writeRequest).result();
  runtime2.log(`[Step 3] PriceOracle updated: marketId=${config.marketId} price=${price6dec}`);
  const dropThreshold = parseFloat(config.anomalyDropThreshold);
  let anomalyDetected = false;
  if (priceSwing >= dropThreshold && prevPrice6dec > 0) {
    anomalyDetected = true;
    runtime2.log(`[Step 4] ⚠ ANOMALY DETECTED: ${(priceSwing * 100).toFixed(2)}% ${swingDir} swing exceeds threshold of ${(dropThreshold * 100).toFixed(0)}%`);
    const BORROWERS_CHECK = config.borrowerAddresses ? config.borrowerAddresses.split(",").map((a) => a.trim()) : ["0x6C9cbb059F5Dbf3f265256a55bbCA0184Dc60564"];
    ...
    for (const borrower of BORROWERS_CHECK) {
      try {
        ...
        const loanRes = evmClient.callContract(runtime2, {
          call: encodeCallMsg({
            from: "0x0000000000000000000000000000000000000000",
        ...
        if (loanRes.data && loanRes.data.length > 0) {
          const decoded = decodeAbiParameters(parseAbiParameters("uint256, uint256, uint256, uint256, bool"), toHex(loanRes.data));
          if (decoded[4]) {
            hasActiveLoans = true;
            break;
          }
    ...
    if (!hasActiveLoans) {
      runtime2.log(`[Step 4] No active loans — skipping AI Guardian analysis (no exposure to protect)`);
    } else {
      runtime2.log(`[Step 4] Active loan(s) found — running AI Guardian pipeline  [via CRE]`);
      ...
      for (let page = 0;page < 2; page++) {
        ...
        runtime2.log(`[Step 4] Fetching comments page ${page + 1}: ${commentsUrl}`);
        try {
          const commentsBody = httpClient.sendRequest(runtime2, (sendRequester) => {
            const res = sendRequester.sendRequest({
              url: commentsUrl,
              method: "GET",
              headers: { Accept: "application/json" }
          ...
          const parsed = JSON.parse(commentsBody);
          ...
          runtime2.log(`[Step 4] Page ${page + 1}: ${parsed.length} comments (total so far: ${allComments.length})`);
          if (parsed.length < COMMENTS_PER_PAGE)
            break;
        } catch {
          runtime2.log(`[Step 4] Could not fetch comments page ${page + 1}`);
          break;
        }
      }
      runtime2.log(`[Step 4] ✓ ${allComments.length} community comments ready — handing off to AI Guardian`);
      await analyzeAnomaly(runtime2, {
        marketId: config.marketId,
        currentPrice: midFloat,
        previousPrice: prevPrice6dec / 1e6,
    ...
    runtime2.log(`[Step 4] No anomaly (swing: ${(priceSwing * 100).toFixed(2)}% | threshold: ${(dropThreshold * 100).toFixed(0)}%) — AI Guardian not triggered`);
  }
  runtime2.log("[Step 5] Checking borrower health factors...");
  ...
  const BORROWERS_TO_MONITOR = config.borrowerAddresses ? config.borrowerAddresses.split(",").map((a) => a.trim()) : ["0x6C9cbb059F5Dbf3f265256a55bbCA0184Dc60564"];
  const WARNING_HF = 1100000000000000000n;
  const LIQUIDATION_HF = 1000000000000000000n;
  let atRiskCount = 0;
  ...
  for (const borrower of BORROWERS_TO_MONITOR) {
    ...
    const loanResult = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: "0x0000000000000000000000000000000000000000",
        to: config.lendingPoolAddress,
        data: loanCalldata
    ...
    if (!loanResult.data || loanResult.data.length === 0) {
      runtime2.log(`[Step 5] ${borrower.slice(0, 10)}... — no active loan`);
      continue;
    }
    const decoded = decodeAbiParameters(parseAbiParameters("uint256, uint256, uint256, uint256, bool"), toHex(loanResult.data));
    ...
    const hfResult = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: "0x0000000000000000000000000000000000000000",
        to: config.lendingPoolAddress,
        data: hfCalldata
    ...
    if (hfResult.data && hfResult.data.length > 0) {
      const [decodedHf] = decodeAbiParameters(parseAbiParameters("uint256"), toHex(hfResult.data));
      hf = decodedHf;
    }
    const hfFloat = Number(hf) / 1000000000000000000;
    const principalUsdc = Number(principal) / 1e6;
    const shortAddr = `${borrower.slice(0, 10)}...`;
    if (hf === BigInt("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")) {
      runtime2.log(`[Step 5] ${shortAddr} — HF: ∞ (no debt or loan just opened)`);
    } else if (hf < LIQUIDATION_HF) {
      liquidatableCount++;
      runtime2.log(`[Step 5] \uD83D\uDD34 LIQUIDATABLE: ${shortAddr} — HF: ${hfFloat.toFixed(4)} | Debt: $${principalUsdc.toFixed(2)}`);
      runtime2.log(`[Step 5]    → ACTION: checkAndEmitRiskEvents will seize collateral (executed by liquidation bot)`);
      runtime2.log(`[Step 5]    → EMAIL: "⚠ Your Sonder position has been liquidated" → ${borrower}`);
    } else if (hf < WARNING_HF) {
      atRiskCount++;
      runtime2.log(`[Step 5] \uD83D\uDFE1 AT RISK:     ${shortAddr} — HF: ${hfFloat.toFixed(4)} | Debt: $${principalUsdc.toFixed(2)}`);
      runtime2.log(`[Step 5]    → EMAIL: "⚠ Your Sonder position is at risk (HF: ${hfFloat.toFixed(4)})" → ${borrower}`);
    } else {
      runtime2.log(`[Step 5] \uD83D\uDFE2 HEALTHY:     ${shortAddr} — HF: ${hfFloat.toFixed(4)} | Debt: $${principalUsdc.toFixed(2)}`);
    }
  }
  runtime2.log(`[Step 5] Summary: ${BORROWERS_TO_MONITOR.length} borrower(s) checked — ${liquidatableCount} liquidatable, ${atRiskCount} at risk`);
  runtime2.log("[Step 6] CRE cycle complete ✓");
  runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
